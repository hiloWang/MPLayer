...
/**
 * 随可滑动内容隐藏显示的 FAB
 * {@link {} R.layout.view_fab (app:layout_behavior="com.hhly.lawyer.widget.ScrollingFABBehavior")}
 */
	...
	@Override public boolean onDependentViewChanged(CoordinatorLayout parent, LinearLayout child, View dependency) {
		...
		if (mAppBarLayout.getY() < 0) { // hide
			if (startAnimation) {
				...
				child.animate().translationY(550f).setDuration(400).setInterpolator(DECELERATE_INTERPOLATOR).start();
			}
